...
async function api(url, data = null, prefix = "game/")
{
    ...
    let $result;
    if (data === null) {
        $result = await axios.get("/rest/" + prefix + url);
    } else {
        $result = await axios.post("/rest/" + prefix + url, $data);
    }
...
let Ludo = {
    ...
    rollDice(game, roll)
    {
        api("roll/"+this.game.sessionPlayer).then((r) => {
            roll = r.dice;
        }).catch((e) => {
            console.log(e);
        });
...
class Piece {
    ...
    move(game)
    {
        api("move/" + Ludo.game.sessionPlayer + "/" + this.pieceIndex).then((r) => {
